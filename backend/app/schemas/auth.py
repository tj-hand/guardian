...
class TokenRequestResponse(BaseModel):
    ...
    email: str = Field(
        ..., description="Email address (masked for security)", example="u***@example.com"
    )
    expires_in_minutes: int = Field(..., description="Token expiry time in minutes", example=15)
...
class TokenValidation(BaseModel):
    ...
    email: EmailStr = Field(..., description="User's email address", example="user@example.com")
    token: str = Field(
        ..., description="6-digit numeric token", min_length=6, max_length=6, example="123456"
    )
...
class TokenValidationResponse(BaseModel):
    ...
    access_token: str = Field(
        ..., description="JWT access token", example="eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9..."
    )
    token_type: str = Field(
        default="bearer", description="Token type (always 'bearer')", example="bearer"
    )
    expires_in: int = Field(..., description="Token expiry time in seconds", example=604800)
...
class UserResponse(BaseModel):
    ...
    id: str = Field(..., description="User UUID", example="123e4567-e89b-12d3-a456-426614174000")
    email: str = Field(..., description="User's email address", example="user@example.com")
    created_at: datetime = Field(
        ..., description="User creation timestamp", example="2025-11-05T12:00:00Z"
    )
...
class AuthErrorResponse(BaseModel):
    ...
    detail: str = Field(..., description="Error detail message", example="Invalid or expired token")
    error_code: Optional[str] = Field(
        None, description="Machine-readable error code", example="TOKEN_EXPIRED"
    )
...
class RateLimitError(BaseModel):
    ...
    retry_after: int = Field(..., description="Seconds until retry is allowed", example=600)
    attempts_remaining: int = Field(
        default=0, description="Number of attempts remaining", example=0
    )
